...
def default_repr(obj):
    ''' Implementación por defecto del método __repr__ de un object. Imprime el
        tipo de obj, y los nombres y valores de sus atributos entre paréntesis. 
    '''
...
def coord_id(column, row):
    ''' Retorna la coordenada en formato letra para la columna y números para 
        la fila. Por ejemplo: coord(1,1) = 'a1', coord(3,7) = 'c7'. 
    '''
    #TODO Columnas mayores estilo planilla electrónica ... z1, aa1, ab1 ...
    return '%s%d' % (__COLUMNS__[column], row+1)

def iterate(func, count=None):
    ''' Generador que llama sin parámetros count veces al callable func. Si
        count es None la repetición es indefinida. 
    '''
...
def board_lines(board, rows, cols):
    ''' Retorna las líneas del tablero con sus casillas.
    '''
