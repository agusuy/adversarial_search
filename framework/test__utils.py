...
class Test__utils(unittest.TestCase): ##########################################
    ''' Testcases for juegos._utils.
    '''

    def test_coord(self):
        ''' Testing juegos._utils.coord_id(column, row)
        '''
        coord_id = juegos._utils.coord_id
        COLUMNS = [l for l in juegos._utils.__COLUMNS__]
        for i, col in zip(xrange(len(COLUMNS)), COLUMNS):
            self.assertEquals(col + str(i+1), coord_id(i, i))

    def test_print_board(self):
        ''' Testing juegos.print_boardjuegos._utils.print_board(board, rows, cols, row_sep, col_sep, joint_sep, row_fmt)
        '''
        print_board = juegos._utils.print_board
        self.assertEquals('A\n', print_board('A', 1, 1, '!', '!', '!'))
    ...
    def test_board_orths(self):
        ''' Testing orthogonal lines (functions board_rows, board_columns, 
            board_orths).
        '''
        board_rows = juegos._utils.board_rows
        board_columns = juegos._utils.board_columns
        board_orths = juegos._utils.board_orths
    ...
    def test_board_diags(self):
        ''' Testing diagonal lines (functions board_pdiags, board_ndiags, 
            board_diags).
        '''
        board_pdiags = juegos._utils.board_pdiags
        board_ndiags = juegos._utils.board_ndiags
        board_diags = juegos._utils.board_diags
